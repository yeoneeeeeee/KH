...
/* 
 * BaseTypeHandelr?
 * 	일반적인 자바의 자료형이 아닌 내가직접 커스텀한 데이터 타입을 처리하기위해 사용되는 클래스.
 *  DB의 특정칼럼에 내가만든 자료형의 값을 대입하고자 할때 사용된다.
 *  
 *  MenuType --> varchar2로 변경하고싶다-> setString함수 오버라이딩.
 *  varchar2 --> MenuType로 반환하고싶다 -> getString함수 오버라이딩
 * 
 * */
@MappedTypes(MenuType.class)// 자바에서사용할 자료형
